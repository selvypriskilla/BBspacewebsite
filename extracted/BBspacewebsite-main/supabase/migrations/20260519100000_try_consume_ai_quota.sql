-- Create an atomic function to reserve AI quota and prevent overruns
BEGIN;
...
BEGIN
  ...
  SELECT s.daily_limit, s.monthly_limit
    INTO daily_limit, monthly_limit
  FROM public.subscriptions s
  WHERE s.user_id = p_user
  LIMIT 1;

  -- Sum current usage for day and month (atomic within function)
  ...
  -- Reserve usage by inserting a log record. The application may later update or add a proper usage record.
  INSERT INTO public.ai_usage_logs(user_id, model, input_tokens, output_tokens, total_tokens, cost_usd, operation, status)
  VALUES (p_user, 'quota_reserve', p_tokens, 0, p_tokens, 0, 'quota_reserve', 'success');
